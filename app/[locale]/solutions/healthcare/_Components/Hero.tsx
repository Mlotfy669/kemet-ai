...
import hero from "@/public/assets/solutions/healthcare/hero.webp";
...
const HeroSection = () => {
  ...
  return (
    <section className="relative h-screen hero-bg mb-20 lg:mb-40">
      ...
      <div className="container h-full mx-auto relative px-6 2xl:py-32 xl:py-6 lg:py-10 md:py-20 py-12">
          ...
          >
            ...
            >
              <motion.h1
                ...
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.4 }}
                className={`font-bold leading-tight relative z-10 ${locale === "ar" ? "md:w-4/5 mb-4 2xl:text-6xl xl:text-[52px] lg:text-[44px] md:text-[36px] text-[27px]" : "2xl:text-7xl xl:text-[50px] lg:text-[44px] md:text-[40px] text-[30px]"}`}
              >
                <Image
                  src={underline}
                  alt="Title underline"
                  className="absolute right-10 top-1/2 -z-1 xl:w-auto lg:w-[250px] md:w-[230px] w-[200px]"
                />
                {t("KEMET Healthcare Solutions")} : <br />
              </motion.h1>
              <motion.span
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ delay: 0.4 }}
                className={`md:w-[70%] flex font-bold leading-loose relative z-10 ${locale === "ar" ? "2xl:text-4xl xl:text-[30px] lg:text-[28px] md:text-[26px] text-[24px]" : "2xl:text-3xl xl:text-[30px] lg:text-[28px] md:text-[26px] text-[24px]"}`}
              >
                {t("Data Sovereignty and Confidential Medical Report Analysis")}
              </motion.span>
            </motion.div>
